...
public class GraphicsDisplay extends JPanel {
	...
	public GraphicsDisplay() {
		...
		graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
				BasicStroke.JOIN_ROUND, 10.0f, new float[] {16, 4, 4, 4, 4, 4, 8, 4, 8, 4}, 0.0f);
	...
	private boolean checkPointCriteria(Double[] point) {
		Double sqrt = Math.sqrt(point[1].intValue());
	...
	protected void paintMarkers(Graphics2D canvas) {
		...
		for (Double[] point: graphicsData) {
			...
			Point2D.Double from = shiftPoint(center, 0, MARKER_SIZE/2);
			Point2D.Double to = shiftPoint(center, 0, -MARKER_SIZE/2);
			...
			from = shiftPoint(center, MARKER_SIZE/2, 0);
			to = shiftPoint(center, -MARKER_SIZE/2, 0);;
